        ...
        if($c){
            ...
            if($table){
                foreach($table as $acc){
                    $accomodation = new Accomodation($acc->ID, $template->getId(), $acc->numero_disponibilita, $acc->start_date, $acc->duration, $acc->template, $acc->address, $acc->type, $acc->description, $acc->category, $acc->name, $acc->link, $acc->photo, $acc->location);
